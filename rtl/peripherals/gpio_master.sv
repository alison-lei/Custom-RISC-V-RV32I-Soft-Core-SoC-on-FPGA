...
module gpio_master (
    ...
    input logic [31:0] mem_addr, // the index, addr after - 0x00001100
    input logic [3:0] buttons, // assign values in top_processor, 4 bits for 4 buttons
    output logic interrupt_req,
    output logic [3:0] gpio_interrupt_id // assume for now 4 buttons
);
    // gpio_mem[0] is for buttons
    logic [31:0] gpio_mem [7:0];
    ...
    always_ff @(posedge clk or posedge reset) begin
        ...
        if (reset) begin
            ...
            gpio_mem[mem_addr] <= {{28'b0}, gpio_interrupt_id}; // delete mem_addr from ports and just hardcode 0 here?
